...
Dim f As Long
    ...
    Randomize Timer
    'Set Game Name
    'G_NAME = GetVar(App.Path & "\Data\data.ini", "Strings", "Name")
    ...
    ' See if another game/service is using that port already.
    If PortInUse(Game_Port) Then
        MsgBox "Selected Port is in use by another program, you may experience network difficulties."
    End If
    
    'set the maximum players
    If GetVar((App.Path & "\data\data.ini"), "Strings", "Name") = "" Then
        GAME_NAME = "Dual Solace's Real-Feel Engine"
    Else
        GAME_NAME = GetVar(App.Path & "\data\data.ini", "Strings", "Name")
    End If
    ...
    ' Set string for Strength
    If GetVar((App.Path & "\data\data.ini"), "Settings", "Strength") = "" Then
        STRING_STRENGTH = "Strength"
    Else
        STRING_STRENGTH = GetVar((App.Path & "\data\data.ini"), "Strings", "Strength")
    End If
    
    ' Set string for Defense
    If GetVar((App.Path & "\data\data.ini"), "Settings", "Defense") = "" Then
        STRING_DEFENSE = "Defense"
    Else
        STRING_DEFENSE = GetVar((App.Path & "\data\data.ini"), "Strings", "Defense")
    End If

    ' Set string for Magic
    If GetVar((App.Path & "\data\data.ini"), "Settings", "Magic") = "" Then
        STRING_MAGIC = "Magic"
    Else
        STRING_MAGIC = GetVar((App.Path & "\data\data.ini"), "Strings", "Magic")
    End If
    
    ' Set string for Speed
    If GetVar((App.Path & "\data\data.ini"), "Settings", "Speed") = "" Then
        STRING_SPEED = "Speed"
    Else
        STRING_SPEED = GetVar((App.Path & "\data\data.ini"), "Strings", "Speed")
    End If
    
    ' Set string for HP
    If GetVar((App.Path & "\data\data.ini"), "Settings", "HP") = "" Then
        STRING_HP = "HP"
    Else
        STRING_HP = GetVar((App.Path & "\data\data.ini"), "Strings", "HP")
    End If
    
        ' Set string for MP
    If GetVar((App.Path & "\data\data.ini"), "Settings", "MP") = "" Then
        STRING_MP = "MP"
    Else
        STRING_MP = GetVar((App.Path & "\data\data.ini"), "Strings", "MP")
    End If
    
        ' Set string for SP
    If GetVar((App.Path & "\data\data.ini"), "Settings", "SP") = "" Then
        STRING_SP = "SP"
    Else
        STRING_SP = GetVar((App.Path & "\data\data.ini"), "Strings", "SP")
    End If
    ...
    Call LoadExps
    Call SetStatus("Loading Library...")
    Call LoadLibrary
    ...
    ' Gets external IP
    Call SetStatus("Configuring Network...")
    frmServer.lblIP.Caption = frmServer.Inet.OpenURL("http://www.whatismyip.org/")
    frmServer.lblPort.Caption = frmServer.Socket(0).LocalPort
    
    frmLoad.lblNetwork.ForeColor = &H8000&
    frmLoad.lblNetwork.Caption = "DONE"
    ...
    Call TextAdd(frmServer.txtText, "News: " & frmServer.Inet.OpenURL("http://www.dualsolace.com/news.txt"), True)
