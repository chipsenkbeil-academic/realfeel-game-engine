...
Dim InvSlot As Byte
    ...
    If LCase(Parse(0)) = "playermove" And Player(Index).GettingMap = NO Then
        ...
        MyScript.ExecuteStatement "main.txt", "OnTile " & Index
        Exit Sub
    End If
    ...
    If LCase(Parse(0)) = "setmotd" Then
        ...
        Call PutVar(App.Path & "\data\data.ini", "Info", "MOTD", Parse(1))
    ...
    ' Trade num
        ...
        Packet = "TRADEGETITEM" & SEP_CHAR & Trim$(CStr(Map(GetPlayerMap(Index)).Shop)) & SEP_CHAR & Trim$(Item(Shop(i).TradeItem(n).GetItem).Name) & SEP_CHAR & Trim$(Item(Shop(i).TradeItem(n).GetItem).Description) & SEP_CHAR & Trim$(Item(Shop(i).TradeItem(n).GiveItem).Name) & SEP_CHAR & CStr(Shop(i).TradeItem(n).GiveValue) & SEP_CHAR & CStr(Shop(i).TradeItem(n).Stock) & SEP_CHAR & CStr(Item(Shop(i).TradeItem(n).GetItem).Type) & SEP_CHAR & CStr(Item(Shop(i).TradeItem(n).GetItem).Data2) & SEP_CHAR
        ...
        If HasItem(Index, Shop(i).TradeItem(n).GiveItem) >= Shop(i).TradeItem(n).GiveValue Then
            ...
            If Shop(i).TradeItem(n).Stock > 0 Then
                ...
                '--------------------Sending packet update------------------- -smchronos
                    Packet = "TRADEGETITEM" & SEP_CHAR & Trim$(CStr(Map(GetPlayerMap(Index)).Shop)) & SEP_CHAR & Trim$(Item(Shop(i).TradeItem(n).GetItem).Name) & SEP_CHAR & Trim$(Item(Shop(i).TradeItem(n).GetItem).Description) & SEP_CHAR & Trim$(Item(Shop(i).TradeItem(n).GiveItem).Name) & SEP_CHAR & CStr(Shop(i).TradeItem(n).GiveItem) & SEP_CHAR & CStr(Shop(i).TradeItem(n).Stock) & SEP_CHAR & CStr(Item(Shop(i).TradeItem(n).GetItem).Type) & SEP_CHAR & CStr(Item(Shop(i).TradeItem(n).GetItem).Data2) & SEP_CHAR
    ...
    If LCase(Parse(0)) = "cast" Then
        ...
        If Player(Index).Target > 0 Then
            If IsPlaying(Player(Index).Target) Then
                'Execute spellcast subroutine
                MyScript.ExecuteStatement "main.txt", "SpellCast " & Index & "," & Player(Index).Target & "," & GetPlayerSpell(Index, n)
            End If
        End If
    ...
    If LCase(Parse(0)) = "removefriend" Then
        For n = 1 To MAX_FRIENDS
            If Player(Index).Char(Player(Index).CharNum).Friends(n) = Parse(1) Then
                Player(Index).Char(Player(Index).CharNum).Friends(n) = ""
    ...
    If LCase(Parse(0)) = "getinfo" Then
        Packet = "SERVERINFO" & SEP_CHAR & GetVar(App.Path & "\Data\data.ini", "Info", "Msg") & SEP_CHAR & END_CHAR
        Call SendDataTo(Index, Packet)
